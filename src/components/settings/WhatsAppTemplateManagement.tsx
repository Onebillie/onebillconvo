      ...
      if (buttons.length > 0) {
        const formattedButtons = buttons.map((btn) => {
          if (btn.type === "URL") {
            ...
            return { type: btn.type, text: btn.text, phone_number: btn.phone_number || "" };
          } else {
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Manage your WhatsApp message templates from Meta Business Manager</CardDescription>
        </CardHeader>
        <CardContent>
          <Tabs defaultValue="existing" className="w-full">
            ...
            <TabsContent value="existing" className="space-y-4">
              <div className="flex items-center justify-between">
                <p className="text-sm text-muted-foreground">{metaTemplates.length} templates from Meta Business Manager</p>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={fetchMetaTemplates} disabled={isLoadingTemplates}>
                    {isLoadingTemplates ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <RefreshCw className="w-4 h-4 mr-2" />}
                    Refresh
                  </Button>
                  <Button variant="default" size="sm" onClick={syncAllTemplates} disabled={isSyncing || metaTemplates.length === 0}>
                    {isSyncing ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Download className="w-4 h-4 mr-2" />}
                    Sync All Approved
                  </Button>
                </div>
              </div>
              ...
              {isLoadingTemplates ? (
                ...
                <div className="text-center py-8 text-muted-foreground">No templates found. Create your first template below.</div>
              ) : (
                      ...
                      {metaTemplates.map((template) => {
                        ...
                        return (
                          <TableRow key={template.id}>
                            ...
                            <TableCell className="max-w-xs truncate text-sm text-muted-foreground">{bodyComponent?.text || "No content"}</TableCell>
                            <TableCell className="text-right">
                              {template.status === "APPROVED" && (
                                <Button variant="ghost" size="sm" onClick={() => syncTemplateToDatabase(template)} disabled={isSyncing}>
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <Input id="name" {...register("name", { required: true })} placeholder="welcome_message" />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select onValueChange={(value) => setValue("language", value)} defaultValue="en_US">
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                ...
                <div className="space-y-2">
                  ...
                  <Input id="header_text" {...register("header_text")} placeholder="Welcome to our service!" />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    rows={4}
                  />
                  <p className="text-sm text-muted-foreground">Use {"{{1}}"}, {"{{2}}"} for variables</p>
                </div>

                <div className="space-y-2">
                  <Label htmlFor="footer_text">Footer Text (Optional)</Label>
                  <Input id="footer_text" {...register("footer_text")} placeholder="Reply STOP to unsubscribe" />
                </div>
